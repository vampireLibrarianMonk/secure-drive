...
ENCRYPTION FORMAT
  The documents are stored in a Cryptomator vault (format 8, cipher
  combination SIV_GCM). This is a publicly documented, open-source
  encryption format:

    - Format documentation: https://docs.cryptomator.org (Security section)
    - Format source: https://github.com/cryptomator/cryptolib
    - KDF: scrypt (parameters are stored in vault\masterkey.cryptomator)

  Unlocking requires exactly one secret: the archive password given to you
  by the archive owner. There is no other key on this drive.

WHERE THE ENCRYPTED DATA LIVES
  vault\                        the complete encrypted vault
  vault\masterkey.cryptomator   password-based key material (do not delete)
  vault\vault.cryptomator       signed vault configuration (do not delete)
  vault\d\                      the encrypted documents and folder structure

RECOVERY PROCEDURE (RECOMMENDED)
  1. On any Windows, macOS, or Linux computer, install the free, open
     source Cryptomator application: https://cryptomator.org
  2. Plug in this USB drive.
  3. In Cryptomator choose "Add Vault" -> "Open existing vault" (or
     "Use masterkey file") and select vault\masterkey.cryptomator on this
     drive.
  4. Enter the archive password. The vault opens as a virtual drive.
  5. Copy the documents you need to the computer.
  6. Lock the vault in Cryptomator before unplugging the drive.

RECOVERY PROCEDURE (WITHOUT INSTALLING SOFTWARE)
  The format is fully documented (links above). A technically competent
  person can decrypt it with any standard crypto library: the KEK is
  scrypt(password, salt, N, r, p=1, 32 bytes); the two AES-256 masterkeys
  are AES-KW wrapped; file names and contents are protected with AES-SIV
  and AES-GCM as specified in the Cryptomator format 8 documentation.

IMPORTANT RULES
  - Do not save, move, rename, or delete any file inside 'vault\'.
  - If the application reports an integrity problem, stop writing to this
    drive, keep it safe, and check whether another replica exists.
  - This file is missing: contact the archive owner; a fresh copy can be
    exported from Setup Mode.

VERSION NOTE
  Format identification: Cryptomator vault format 8, masterkey file
  version 999, KDF parameters as recorded in masterkey.cryptomator.
"@
